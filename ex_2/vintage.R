...
# 5
matplot(t(N_ij), type = "b", pch = 16, col = rainbow(12), lwd = 3, lty = 1,
        xlab = "Évek",
...
for (year in 2009:2020){
  ...
  change_vector <- c(NA,
                     (1 - N_dt[Start_year == year]$Cum_prob[2:12]) /
    (1 - N_dt[Start_year == year]$Cum_prob[1:11]) - 1)
...
# A Cum_pro_change adja meg, hogy adott évben kibocsájtott hitelek, a különböző években
# mekkora csökkenést lehetett tapasztalni a portfólióban.
